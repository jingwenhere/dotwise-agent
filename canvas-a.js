(() => {
  ...
  // Canvas A starts with Interaction B behavior, in its own script and stylesheet.
  const isInteractionB = true;
  document.documentElement.dataset.interaction = 'b';
  ...
  const openPinnedCanvas = document.getElementById('openPinnedCanvas');
  const openTaskCanvas = document.getElementById('openTaskCanvas');
  const openDocumentPage = document.getElementById('openDocumentPage');
  const canvasChatButton = document.getElementById('canvasChatButton');
  const canvasZoomButton = document.getElementById('canvasZoomButton');
  ...
  let toastTimer = null;
  let agentWasCollapsedOnCanvasEntry = false;
  ...
  function setActiveWorkspaceItem(item) {
    document.querySelectorAll('.sidebar-scroll .nav-item.active').forEach((button) => {
  ...
  function showCanvasPage(item) {
    if (canvasWorkspace.hidden) agentWasCollapsedOnCanvasEntry = contentShell.classList.contains('is-agent-collapsed');
    ...
    agentPanel.inert = true;
    contentShell.classList.add('is-canvas-view');
    contentShell.classList.remove('is-canvas-chat-open');
    document.body.classList.add('is-canvas-view');
    canvasChatButton.setAttribute('aria-expanded', 'false');
    setActiveWorkspaceItem(item);
    dismissGuide();
  }

  function showDocumentPage() {
    canvasWorkspace.hidden = true;
    documentWorkspace.hidden = false;
    agentPanel.hidden = false;
    contentShell.classList.remove('is-canvas-view', 'is-canvas-chat-open');
    document.body.classList.remove('is-canvas-view');
    canvasChatButton.setAttribute('aria-expanded', 'false');
    setActiveWorkspaceItem(openDocumentPage);
    setAgentPanelCollapsed(agentWasCollapsedOnCanvasEntry, { focus: false });
  }
  ...
  collapseAgentPanelButton.addEventListener('click', () => {
    ...
    if (contentShell.classList.contains('is-canvas-view')) {
      contentShell.classList.remove('is-canvas-chat-open');
      agentPanel.hidden = true;
      canvasChatButton.setAttribute('aria-expanded', 'false');
    }
  ...
  openPinnedCanvas.addEventListener('click', () => showCanvasPage(openPinnedCanvas));
  openTaskCanvas.addEventListener('click', () => showCanvasPage(openTaskCanvas));
  openDocumentPage.addEventListener('click', showDocumentPage);
  canvasChatButton.addEventListener('click', () => {
    const isOpen = contentShell.classList.toggle('is-canvas-chat-open');
  ...
  canvasZoomButton.addEventListener('click', () => {
    ...
    canvasZoomButton.textContent = `${next}%`;
    canvasZoomButton.setAttribute('aria-label', `Canvas zoom ${next} percent`);
    canvasBoard.style.setProperty('--canvas-zoom', String(next / 100));
  });
